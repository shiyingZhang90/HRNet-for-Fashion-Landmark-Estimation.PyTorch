...
def load_keypoints(source_keypoint_path='', target_keypoint_path='',
    ...
    print('s', raw_source_keypoint)
    print('t', raw_target_keypoint)
...
def get_midpoint(point1, point2, x_val):
    if point2[0] != point1[0]:
        ...
        return np.array([x_val, point2[0]]).reshape(1, 2)
...
def main(source_img_root='./data', target_img_root='./data', source_name='image_2', target_name='image_1',
    ...
    source_img = source_img * mask + 255 * (1 - mask)
    ...
    warp_img = np.clip(warp_img * 1.25, 0, 255).astype(np.uint8)  ## brigter
