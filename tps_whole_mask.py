...
def clip_feather(mask):
    mask=cv2.blur(mask,(3,3))
    mask = np.clip(mask * 2 - 1, 0, 1)
    return mask[:,:,np.newaxis]
def gauss_blur(img,mask,alpha=0.7):
    band=morpho(mask,1,True)*(1-morpho(mask,1,False))
    blur=cv2.GaussianBlur(img,(3,3),0)
    blur=alpha*blur+(1-alpha)*img
...
def main(source_img_root='./data', target_img_root='./data', source_name='image_2', target_name='image_1',
    ...
    cloth_mask_fn=os.path.join(target_img_root.replace('model_mask','mask'),target_name)
    ...
    # mask*=(mask>0.05).astype(np.float)
    mask=mask[:,:,np.newaxis]
    # mask=clip_feather(mask)
...
if __name__ == '__main__':
    ...
    for source in os.listdir(args.logo_root):
        for target_folder in os.listdir(args.model_root):
            ...
            print("target_root is ", target_root)
